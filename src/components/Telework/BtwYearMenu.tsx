...
const BtwYearMenu: React.FC<{ onSelectionChange: (selections: { week: weekOption, employment: Option, startYear: string, endYear: string }) => void }> = ({ onSelectionChange }) => {
    ...
    useEffect(() => {
        if (isMaxYearLoaded) {
            onSelectionChange({ week: weekValue, employment: employmentValue, startYear: startYear.value, endYear: endYear.value });
        }
    }, [isMaxYearLoaded, weekValue, employmentValue, startYear, endYear, onSelectionChange]);
